  ...
  return (
    <motion.div
      animate={{
        ...
        boxShadow: visible
          ? "0 0 24px rgba(34, 42, 53, 0.04), 0 1px 1px rgba(0, 0, 0, 0.03), 0 0 0 1px rgba(34, 42, 53, 0.03), 0 0 4px rgba(34, 42, 53, 0.05), 0 8px 40px rgba(47, 48, 55, 0.03), 0 1px 0 rgba(255, 255, 255, 0.05) inset"
          : "none",
      ...
      className={cn(
        ...
        visible ? "bg-white dark:bg-neutral-900" : "",
        className
      )}
    >
      {children}
    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-black hover:text-gray-600 dark:text-white dark:hover:text-gray-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-gray-100 dark:bg-neutral-700"
            />
          )}
...
export const MobileNavToggle = ({ isOpen, onClick }: { isOpen: boolean; onClick: () => void }) => {
  return (
    <button className="text-black dark:text-white">
      {isOpen ? (
        <IconX size={24} onClick={onClick} />
      ) : (
        <IconMenu2 size={24} onClick={onClick} />
      )}
    </button>
  );
};
...
export const NavbarLogo = () => {
  return (
    ...
    >
      ...
      />
      <span className="font-medium">Beauty Saloon</span>
    </a>
  );
};
